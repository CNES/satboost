...
function handleHistory(details){
  if(details.frameId === 0) {
      ...
      browser.tabs.get(details.tabId, function(tab) {
          if(tab.url === details.url && tab.url.match(/http(s?):.+youtube.+watch.+/)) {
              // Transmission id = 1
              browser.tabs.sendMessage(tab.id, {txId : 1, data: tab.url});
          }
      });
  }
}
...
function handleMessages(request, sender, sendResponse){
  //Message related to Youtube Stats
  if (request.txId == 2) {
    var xhr = new XMLHttpRequest();
...
//Add Listeners
browser.webNavigation.onHistoryStateUpdated.addListener(handleHistory);
